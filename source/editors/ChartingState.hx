...
class ChartingState extends MusicBeatState
{
	...
	override function create()
	{
		...
		for (i in 0...tipTextArray.length)
		{
			...
			tipText.y += i * 14;
			tipText.setFormat(Paths.font("vcr.ttf"), 16, FlxColor.WHITE, LEFT);
	...
	function addSongUI():Void
	{
		...
		var reloadSongJson:FlxButton = new FlxButton(reloadSong.x, saveButton.y + 30, "Reload JSON", function() { loadJson(_song.song.toLowerCase()); });
		...
		var clear_events:FlxButton = new FlxButton(320, 310, 'Clear events', function() { clearEvents(); });
		...
		#if MODS_ALLOWED
		var stageDirectories:Array<String> = [Paths.mods('stages/'), Paths.mods(Paths.currentModDirectory + '/stages/'), Paths.getPreloadPath('stages/')];
		#else
		var stageDirectories:Array<String> = [Paths.getPreloadPath('stages/')];
		#end
		...
		check_bfskin.callback = function() { _song.allowBFskin = check_bfskin.checked; };
		...
		check_gfskin.callback = function() { _song.allowGFskin = check_gfskin.checked; };
		...
		check_pet.callback = function() { _song.allowPet = check_pet.checked; };
	...
	function addSectionUI():Void
	{
		...
		var copyButton:FlxButton = new FlxButton(10, 150, "Copy Section", function()
		{
			...
			for (i in 0..._song.notes[curSection].sectionNotes.length)
				notesCopied.push(_song.notes[curSection].sectionNotes[i]);
		});
	...
	function addNoteUI():Void
	{
		...
		#if LUA_ALLOWED
		var luaDirectories:Array<String> = [Paths.mods('custom_notetypes/'), Paths.mods(Paths.currentModDirectory + '/custom_notetypes/')];
		for (i in 0...luaDirectories.length)
		{
			var directory:String = luaDirectories[i];
			if (FileSystem.exists(directory))
			{
				for (file in FileSystem.readDirectory(directory))
				{
	...
	function addEventsUI():Void
	{
		...
		var eventDirectories:Array<String> = [Paths.mods('custom_events/'), Paths.mods(Paths.currentModDirectory + '/custom_events/')];
		for (i in 0...eventDirectories.length)
		{
			var directory:String = eventDirectories[i];
			if (FileSystem.exists(directory))
			{
				for (file in FileSystem.readDirectory(directory))
				{
		...
		tab_group_event.add(new FlxText(20, 30, 0, "Event:"));
	...
	function addChartingUI()
	{
		...
		waveformEnabled.callback = function() { updateWaveform(); };
		...
		waveformUseInstrumental.callback = function() { updateWaveform(); };
		#end
	...
	{
		if (audioBuffers[0] != null) audioBuffers[0].dispose();
		audioBuffers[0] = null;
		#if MODS_ALLOWED
		...
		if (audioBuffers[1] != null) audioBuffers[1].dispose();
		audioBuffers[1] = null;
		#if MODS_ALLOWED
	...
	function setupNoteData(i:Array<Dynamic>, isNextSection:Bool):Note
	{
		...
		if (isNextSection && _song.notes[curSection].mustHitSection != _song.notes[curSection + 1].mustHitSection)
		{
			...
			else if (daNoteInfo > -1) note.x += GRID_SIZE * 4;
		}

		note.y = (GRID_SIZE * (isNextSection ? 16 : 0)) * zoomList[curZoom]
			+ Math.floor(getYfromStrum((daStrumTime - sectionStartTime(isNextSection ? 1 : 0)) % (Conductor.stepCrochet * _song.notes[curSection].lengthInSteps), false));
		return note;
	}
	...
	function loadLevel():Void { FlxG.log.add(_song.notes); }
