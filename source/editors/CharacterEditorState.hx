...
class CharacterEditorState extends MusicBeatState
{
	...
	function addAnimationsUI()
	{
		...
		ghostDropDown = new FlxUIDropDownMenuCustom(animationDropDown.x + 150, animationDropDown.y,
			FlxUIDropDownMenuCustom.makeStrIdLabelArray([''], true), function(pressed:String)
			{
				var selectedAnimation:Int = Std.parseInt(pressed);
	...
	override function update(elapsed:Float)
	{
		if (char.animationsArray[curAnim] != null)
		{
			...
			var curAnimObj:FlxAnimation = char.animation.getByName(char.animationsArray[curAnim].anim);
			if (curAnimObj == null || curAnimObj.frames.length < 1)
				textAnim.text += ' (ERROR!)';
		}
		else
		{
